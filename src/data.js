...
export const data = dataOrigin.data.advisorProfileCollection.items.map(
  (item) => {
    return {
      id: item.sys.id,
      ...
      categories: [
        ...new Set(item.categoriesCollection.items.map((i) => i.displayName)),
      ],
      skills: [
        ...new Set(item.skillsCollection.items.map((i) => i.displayName)),
      ],
      services: [...new Set(item.servicesCollection.items.map((i) => i.name))],
    };
  }
);
